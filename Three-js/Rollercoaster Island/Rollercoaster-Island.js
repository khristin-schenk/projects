...
function init() {
  ...
  camera.update = function (time) {
    if (typeof track !== "undefined" && å.track.enabled) {
      ...
      var camera_position = centerPoint(position_l, position_r, 0.5);
      var lookAt_position = centerPoint(
        lookAt_position_l,
        lookAt_position_r,
        0.5
      );
  ...
  function draw_inbetween_line(positions) {
    ...
    positions[0].forEach(function (pos, index) {
      ...
      line_geometry.vertices.push(
        ...
        new THREE.Vector3(between_p.x, between_p.y - 0.1, between_p.z),
        new THREE.Vector3(
          positions[1][index].x,
          positions[1][index].y,
          positions[1][index].z
  ...
  loader.load(å.model.monster, function (geometry, material) {
    ...
    monster.update = function (time) {
      ...
      this.rotation.x +=
        0.01 * this.direction * (1.9 - Math.abs(this.rotation.x));
